...
def arpSpoof():
	...
	if fg==0:
	    ...
	    List_IP.insert(0,packets[IP].dst)
    ...
    def Regular_Expression(r, s):
        if r == 1:
            ...
            Email_Pattern = re.compile(r'[a-zA-Z][a-zA-Z0-9_]+@[a-zA-Z-_]+\.[a-z]+\.?[a-z]+\.?[a-z]+')
            Email_Matches = Email_Pattern.finditer(s)
            for Email in Email_Matches:
                List_Email.insert(0, Email.group(0))
                return Email.group(0)
        elif r == 4:
            Mobile_Pattern = re.compile(r'(\d{10})|(\d{4}\s\d{6})|(.\d{4}.\d{6})|(0\d{3}-\d{6})')
...
Label_IP = Label(root, text="IP:", fg="black")
